...
const CLASSIC_SCORE_FIELDS = Object.keys(HIGHSCORE_FIELDS).filter((field) => !HIGHSCORE_ICONS[field]);
const FALLBACK_VERIFICATION_EMAIL_HTML = `
  <p>Your Photo Memory account is waiting. Use the button below to finish verifying your email and start training.</p>
  <p><a href="../verify.html?preview=true" style="display:inline-block;background:#f6a21a;color:#191816;border:3px solid #1d1a17;border-radius:999px;padding:12px 18px;text-decoration:none;font-weight:800;">Activate account</a></p>
  <p style="color:#706a60;">If this was not you, you can ignore this email.</p>
`;
const BUILT_IN_MAIL_PRESETS = [
  {
    id: 'unverified-reminder',
    name: 'Unverified reminders',
    description: 'Loads the account activation reminder template from the Edge Function.',
    subject: 'Complete your Photo Memory account',
    audience: 'unverified',
    html: FALLBACK_VERIFICATION_EMAIL_HTML
  }
];
...
function renderHighscores() {
  ...
  renderHighscoreGroup('#memory-highscores-grid', MEMORY_TYPE_SCORE_FIELDS);
  renderHighscoreGroup('#classic-highscores-grid', CLASSIC_SCORE_FIELDS);
}

function renderHighscoreGroup(selector, fields) {
...
function renderLeaderboard() {
  ...
  $('#leaderboard-list').innerHTML = ranked.map((user, index) => `
    <tr class="${user.id === state.currentProfile?.id ? 'current-user-row' : ''} ${user.id === state.comparisonUserId ? 'comparison-user-row' : ''}">
  ...
  `).join('') || '<tr><td colspan="4" class="empty-table">No leaderboard data yet.</td></tr>';
}
...
function buildHighscoresReport() {
  ...
  return `
    ...
    <h3>Memory type-based games</h3>
    <table><thead><tr><th>Game</th><th>You</th>${compareHeader}</tr></thead><tbody>${scoreReportRows(MEMORY_TYPE_SCORE_FIELDS, comparisonUser)}</tbody></table>
    <h3>Classics</h3>
    <table><thead><tr><th>Game</th><th>You</th>${compareHeader}</tr></thead><tbody>${scoreReportRows(CLASSIC_SCORE_FIELDS, comparisonUser)}</tbody></table>
  `;
}
...
function getAudienceUserIds(kind) {
  if (kind === 'selected') return Array.from(state.selectedUserIds);
  if (kind === 'filtered') return getFilteredAdminUsers().map((user) => user.id);
  if (kind === 'unverified') return state.adminUsers.filter((user) => !isVerified(user)).map((user) => user.id);
  return state.adminUsers.map((user) => user.id);
}
...
async function sendBulkMail() {
  const audienceKind = $('#mail-audience').value;
  const userIds = getAudienceUserIds(audienceKind);
  if (!userIds.length) {
  ...
  try {
    ...
    const emailPayload = {
      ...
      automate: $('#mail-automate').checked
    };

    const promises = [api('sendBulkEmail', emailPayload)];

    // Send push notification concurrently if checked
    if ($('#mail-send-push').checked) {
      const title = $('#mail-push-title').value.trim() || 'New updates available!';
      const body = $('#mail-push-body').value.trim() || 'Check your email inbox or log in to Photo Memory now!';
    ...
    toast(`Campaign dispatched successfully to ${userIds.length} user${userIds.length === 1 ? '' : 's'}.`);
...
let mailEditorMode = 'visual'; // 'visual' or 'html'
...
function renderMailPresetCards() {
  ...
  const customPresets = getMailPresets();
  const cards = [
    ...BUILT_IN_MAIL_PRESETS.map((preset) => ({ ...preset, token: `builtin:${preset.id}`, builtIn: true })),
    ...customPresets.map((preset, index) => ({ ...preset, token: `custom:${index}`, builtIn: false }))
  ];

  $('#mail-preset-count').textContent = cards.length;
  list.innerHTML = cards.map((preset) => {
    const preview = preset.description || stripHtmlText(preset.html || '').slice(0, 118) || 'No preview text yet.';
    ...
    return `
      <article class="mail-preset-card${activeClass}">
        <button class="mail-preset-main" type="button" data-mail-preset-load="${escapeHtml(preset.token)}">
          <span class="mail-preset-kicker">${preset.builtIn ? 'Built in' : 'Saved preset'}</span>
  ...
  if (token?.startsWith('builtin:')) {
    ...
    if (presetId === 'unverified-reminder') {
      try {
        ...
        toast('Could not load the Edge Function template, so the local preview copy was loaded.', true);
      }
    }
...
function applyMailPreset(preset, token = '') {
  ...
  $('#mail-automate').checked = Boolean(preset.automate);
  $('#mail-send-push').checked = Boolean(preset.sendPush);
  $('#mail-push-title').value = preset.pushTitle || '';
  $('#mail-push-body').value = preset.pushBody || '';

  const html = preset.html || '';
  $('#mail-editor').innerHTML = html;
  $('#mail-editor-source').value = html;
  $('#mail-push-details').classList.toggle('active', Boolean(preset.sendPush));
  $('#mail-push-details').style.display = preset.sendPush ? 'grid' : 'none';
...
function saveCurrentMailPreset() {
  ...
  const newPreset = {
    ...
    automate: $('#mail-automate').checked,
    sendPush: $('#mail-send-push').checked,
    pushTitle: $('#mail-push-title').value.trim(),
    pushBody: $('#mail-push-body').value.trim()
  };
...
function initMailStudio() {
  ...
  // Concurrent Push Notification toggle
  $('#mail-send-push').addEventListener('change', (e) => {
    const pushDetails = $('#mail-push-details');
    if (pushDetails) {
      pushDetails.classList.toggle('active', e.target.checked);
      pushDetails.style.display = e.target.checked ? 'grid' : 'none';
    }
  });
...
async function previewUnverifiedEmail() {
  const button = $('#preview-unverified-email-btn');
  const frame = $('#preview-email-frame');
  ...
  try {
    ...
    $('#preview-email-dialog').showModal();
  } catch (error) {
    toast(error.message, true);
  } finally {
    setBusy(button, false);
...
function resizePreviewFrame(frame) {
  try {
    ...
    frame.style.height = `${Math.min(Math.max(height, 420), window.innerHeight - 210)}px`;
  } catch (_error) {
    frame.style.height = '620px';
  }
}

async function sendUnverifiedReminders() {
  const selectedIds = Array.from(state.selectedUserIds);
  const userIds = selectedIds.length
    ? selectedIds
    : state.adminUsers.filter((user) => !isVerified(user)).map((user) => user.id);
  const unverifiedCount = state.adminUsers.filter((user) => userIds.includes(user.id) && !isVerified(user)).length;

  if (!unverifiedCount) {
    toast('No unverified users selected or visible.', true);
    return;
  }

  if (!confirm(`Send verification reminder to ${unverifiedCount} unverified user${unverifiedCount === 1 ? '' : 's'}?`)) {
    return;
  }

  const button = $('#send-unverified-email-btn');
  setBusy(button, true, 'Sending...');

  try {
    const result = await api('sendUnverifiedVerificationEmails', { userIds });
    const failed = result.failed?.length || 0;
    const firstFailure = failed && result.failed[0]?.error ? ` First error: ${result.failed[0].error}` : '';
    toast(`Sent ${result.sent || 0} reminder${result.sent === 1 ? '' : 's'}. ${failed} failed.${firstFailure}`, failed > 0);
    await loadAdminDashboard();
  } catch (error) {
    toast(error.message, true);
  } finally {
...
async function createNotification() {
  ...
  const userIds = getAudienceUserIds(audience);
  if (audience !== 'all' && !userIds.length) {
...
function attachEventListeners() {
  ...
  $('#clear-comparison-btn').addEventListener('click', () => {
  ...
  $('#preview-unverified-email-btn').addEventListener('click', previewUnverifiedEmail);
  $('#send-unverified-email-btn').addEventListener('click', sendUnverifiedReminders);
